...
export default function ProjectFilters({
  ...
  const cItems = useMemo(
    () =>
      categories.map((c) => {
        return { label: c.label, value: c.key } as TFilter;
      }),
    [categories],
  );
  const lItems = useMemo(
    () =>
      languages.map((l) => {
        return { label: l.label, value: l.key } as TFilter;
      }),
    [languages],
  );
  const tItem = useMemo(
    () =>
      technologies.map((t) => {
        return { label: t.label, value: t.key } as TFilter;
      }),
    [technologies],
  );
  ...
  const handler = (data: TFilter[], char: string) => {
    if (char === "l") {
      ...
      handleCategory(
        data.map((item) => {
          return { label: item.label, key: item.value } as ICategoryFilter;
        }),
      );
      return;
    } else if (char === "t") {
      handleSkill(
        technologies.filter((s) => data.some((d) => d.value === s.key)),
      );
      return;
    } else {
      handleDate(data);
