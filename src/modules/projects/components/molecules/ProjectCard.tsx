...
export default function ProjectCard({project, locale, translation}: Props){
  return(
    <li className="project_card">
      <img src={project.picture} alt={`${project.collection?.name} cover image`} className="project_card-image"/>
      <div className="px-2 py-3 flex flex-col gap-1">
        <h5 className="first-letter:capitalize font-bold text-2xl">{project.collection?.name}</h5>
        ...
        <p className="py-2 font-light">{project.collection?.description}</p>
        <TechPrevList label={translation.languages} items={project.languages} />
        <TechPrevList label={translation.technologies} items={project.technologies} />
        <figure className="w-full h-[1px] bg-border my-1"/>
        <div className="flex items-center justify-between gap-1">
          <div className="flex items-center justify-start gap-1">
            <figure className="text-xs">{"</>"}</figure>
            <h6 className="font-light text-sm">{project.collection?.architecture}</h6>
          </div>
          <div className="font-light text-sm hover:text-primary-focus select-none flex items-center gap-1.5 capitalize">
